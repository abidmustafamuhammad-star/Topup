...
// Rate limiter persisten di Mongo (aman untuk Vercel/serverless)
app.use('/api/', async (req, res, next) => {
  try {
    const key = `ratelimit:${getIp(req)}`;
    const allowed = await checkRateLimit(key, config.security.rateLimitWindow, config.security.rateLimitMax);
    if (!allowed) return res.status(429).json({ success: false, error: 'Terlalu banyak request. Tunggu sebentar.' });
    next();
  } catch (e) {
    log.error('rateLimit gagal, fail-open', e.message);
    next();
  }
});
...
app.get('/deposit',       pub('deposit.html'));
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    if (lock?.lock_until && new Date() < new Date(lock.lock_until)) {
      const wait = Math.ceil((new Date(lock.lock_until) - Date.now()) / 60000);
    ...
    if (!valid) {
      await registerFailedLogin(ip, config.security.maxLoginAttempts, config.security.lockoutMs);
...
app.get('/api/catalog/countries', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    log.info('catalog/countries result:', JSON.stringify(result).slice(0, 500));
    res.json({ success: true, ...result });
  }
  catch (e) { log.error('catalog/countries error:', e.message); res.status(502).json({ success: false, error: 'Gagal mengambil daftar negara.' }); }
});
...
async function fulfillOrder(orderId) {
  ...
  try {
    ...
    await Order.updateOne({ _id: orderId }, {
      status: 'number_issued', phone_number: phone, rumahotp_order_id: rumahOrderId,
    ...
    const locked = await Order.findOneAndUpdate(
      { _id: orderId, refunded: false },
      { status: 'cancelled', refunded: true },
      { new: true }
    );
    if (locked) await refundOrderBalance(locked.user_id, locked.price, locked._id, 'Gagal ambil nomor dari provider');
  }
}

// ============================================================
// DEPOSIT — isi saldo via QRIS Pakasir
// ============================================================
app.post('/api/deposit/create', authMiddleware, async (req, res) => {
  const ip = getIp(req);
  try {
    const amount = parseFloat(req.body.amount);
    if (!amount || isNaN(amount)) return res.status(400).json({ success: false, error: 'Jumlah deposit tidak valid.' });
    if (amount < config.pricing.minDeposit) return res.status(400).json({ success: false, error: `Deposit minimal ${formatRp(config.pricing.minDeposit)}.` });
    if (amount > config.pricing.maxDeposit) return res.status(400).json({ success: false, error: `Deposit maksimal ${formatRp(config.pricing.maxDeposit)}.` });

    const pakasirId = genPakasirOrderId('DEP');
    let qrData;
    try {
      qrData = await pakasir.createQris(pakasirId, amount);
    } catch (e) {
      log.error('deposit qris', e.message);
      return res.status(502).json({ success: false, error: 'Gagal membuat pembayaran QRIS. Coba lagi.' });
    }

    await Transaction.create({
      order_id: null, type: 'deposit', user_id: req.user._id,
      pakasir_order_id: pakasirId, amount,
      qr_string: qrData?.qr_string || '',
      expired_at: qrData?.expired_at ? new Date(qrData.expired_at) : new Date(Date.now() + 5 * 60000),
    });

    await auditLog(null, req.user._id, 'deposit_create', { amount, pakasir_order_id: pakasirId }, ip);
    res.json({ success: true, pakasir_order_id: pakasirId, amount, qr_string: qrData?.qr_string, expired_at: qrData?.expired_at });
  } catch (e) { log.error('deposit/create', e.message); res.status(500).json({ success: false, error: 'Gagal membuat deposit.' }); }
});

/** Cek status pembayaran deposit (polling dari client) */
app.get('/api/deposit/:pakasirId/status', authMiddleware, async (req, res) => {
  try {
    const tx = await Transaction.findOne({ pakasir_order_id: req.params.pakasirId, user_id: req.user._id, type: 'deposit' }).lean();
...
/** User buat pesanan — dibayar langsung dari saldo (isi saldo dulu via /deposit) */
app.post('/api/order/create', authMiddleware, async (req, res) => {
  const ip = getIp(req);
  try {
    const { service_id, service_name, country, country_name, provider_id, operator_id, number_id } = req.body;
    ...
    let canonical;
    try { canonical = await resolveCanonicalPrice(service_id, number_id, provider_id); }
    catch (e) { log.error('resolveCanonicalPrice', e.message); return res.status(502).json({ success: false, error: 'Gagal memverifikasi harga ke provider.' }); }
    if (!canonical) return res.status(400).json({ success: false, error: 'Kombinasi layanan/negara/provider tidak valid.' });
    if (canonical.stock != null && Number(canonical.stock) <= 0) return res.status(409).json({ success: false, error: 'Stok untuk pilihan ini sedang habis.' });

    const base_price = canonical.base_price;
    ...
    if (!debited) return res.status(402).json({ success: false, error: 'Saldo tidak cukup. Silakan isi saldo terlebih dahulu.', need_deposit: true });
    ...
    try {
      newOrder = await Order.create({
        ...
        number_id, base_price, price, status: 'paid',
      });
    } catch (e) {
      await User.updateOne({ _id: req.user._id }, { $inc: { balance: price } });
      throw e;
    }

    await balanceLog(null, req.user._id, 'debit', price, debited.balance + price, debited.balance, 'order', newOrder._id, 'Pembelian nomor virtual');
    await auditLog(null, req.user._id, 'order_create', { order_id: newOrder._id, service_id, price }, ip);

    fulfillOrder(newOrder._id).catch(e => log.error('fulfillOrder async', e.message));

    res.json({ success: true, order_id: newOrder._id, price, balance: debited.balance });
...
/** Cancel order — otomatis refund saldo */
app.post('/api/order/:id/cancel', authMiddleware, async (req, res) => {
  const ip = getIp(req);
  try {
    const order = await Order.findOne({ _id: req.params.id, user_id: req.user._id });
    if (!order) return res.status(404).json({ success: false, error: 'Pesanan tidak ditemukan.' });
    if (!['paid', 'number_issued'].includes(order.status))
    ...
    if (locked) await refundOrderBalance(locked.user_id, locked.price, locked._id, 'Cancel manual oleh user');

    await auditLog(null, req.user._id, 'order_cancel', { order_id: order._id }, ip);
    res.json({ success: true });
  } catch (e) { log.error('order/cancel', e.message); res.status(500).json({ success: false, error: 'Gagal membatalkan.' }); }
});
...
app.post('/webhook/pakasir', async (req, res) => {
  ...
  try {
    ...
    let detail;
    try {
      detail = await pakasir.getDetail(order_id, amount);
    } catch (e) {
      // JANGAN anggap sukses kalau verifikasi ke Pakasir gagal — tahan untuk review manual
      log.error('pakasir verify gagal, transaksi ditahan', e.message);
      await auditLog(null, tx.user_id, 'payment_verify_failed', { order_id, amount }, ip, 'failed');
      return;
    }

    const verified = (detail?.status === 'success' || detail?.status === 'completed')
      && Number(detail?.amount) === Number(amount)
      && Number(tx.amount) === Number(amount);

    if (!verified) {
      log.warn('pakasir webhook: verifikasi gagal / amount mismatch', { order_id, amount, tx_amount: tx.amount });
      await auditLog(null, tx.user_id, 'payment_verify_mismatch', { order_id, amount }, ip, 'failed');
      return;
    }
    ...
    // Sekarang webhook Pakasir hanya dipakai untuk deposit saldo
    const u = await User.findById(tx.user_id);
    if (u) {
      const bal = u.balance;
      await User.updateOne({ _id: tx.user_id }, { $inc: { balance: amount } });
      await balanceLog(null, tx.user_id, 'credit', amount, bal, Number(bal) + Number(amount), 'deposit', tx._id, 'Deposit via QRIS Pakasir');
      if (u.telegram_chat_id) {
        await tg.sendPaymentNotification(u.telegram_chat_id, amount, config.site.name);
      }
    }
    await auditLog(null, tx.user_id, 'deposit_success', { order_id, amount }, ip);
  ...
  try {
    ...
    for (const o of staleOrders) {
      try {
        ...
        const locked = await Order.findOneAndUpdate(
          { _id: o._id, refunded: false, status: { $in: ['paid', 'number_issued'] } },
          { status: 'expired', refunded: true },
          { new: true }
        );
        if (locked) await refundOrderBalance(locked.user_id, locked.price, locked._id, 'Auto-cancel: waktu habis / OTP tidak masuk');
        cancelled++;
...
app.get('/api/admin/users/:id', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const u = await User.findById(req.params.id)
      .select('username email role status balance telegram_chat_id created_at')
      .lean();
    if (!u) return res.status(404).json({ success: false, error: 'User tidak ditemukan.' });
    res.json({ success: true, user: { ...u, id: u._id } });
  } catch (e) { res.status(400).json({ success: false, error: 'ID user tidak valid.' }); }
});
